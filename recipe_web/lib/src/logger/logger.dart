...
@Injectable()
class Recipes2Logger {
  Logger _logger;
  String _loggerName = '';
  LoggerService _loggerService;

  Recipes2Logger(this._loggerService) : super() {
    log(Level.FINE, 'Recipes2Logger() _loggerService = $_loggerService');
  }

  void set loggerName(String newName) {
    _logger = new Logger(newName);
    _loggerName = newName;
    log(Level.FINE, 'Recipes2Logger() set loggerName = $newName');
  }
...
//    print('send this to the database: $level, $message');
  ...
  String toString() {
    return 'Recipes2Logger($_loggerName) level: ${_logger.level} fullName: ${_logger.fullName}';
  }
}