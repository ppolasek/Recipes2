...
abstract class RecipeService extends WebService {
  ...
  RecipeService(Recipes2Logger logger, BrowserClient http, Recipes2AppConfig config) : super(logger, http, config);

  Future<Recipe> getRecipe(int id);
  Future<Recipe> getRecipeWithHistory(int id);
  Future<List<Recipe>> getRecipesForCategory(int categoryId);
...
class WebRecipeService extends RecipeService {
  ...
  WebRecipeService(this._log, BrowserClient http, Recipes2AppConfig config) : super(_log, http, config) {
  ...
  @override
  Future<List<Recipe>> getRecipesForCategory(int categoryId) async {
    final response = await makeTheCall('getRecipesForCategory', categoryId);

    var recipes = Recipe.fromList(response);
    _log.finer('getRecipesForCategory() recipes = $recipes');
    return recipes;
  }
